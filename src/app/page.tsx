...
export default function HomePage() {
  ...
  const handleCreateEntity = async (entityType: LocalEntityType, formData: CompanyFormData | ContactFormData) => {
    ...
    try {
      ...
      toast(<ToastPositive title={`${entityType} created successfully!🎉`} />,{
        unstyled: true,
      });
    } catch (error) {
      toast(
  ...
  const handleUpdateEntity = async (formData: CompanyFormData | ContactFormData, id: string | undefined) => {
    const input = {
      id,
      ...formData,
      entityType: entityType === LocalEntityType.COMPANY ? EntityType.COMPANY : EntityType.CONTACT,
    ...
    try {
      ...
      if (res?.errors && res?.errors?.length > 0) {
        toast(
          <ToastNegative title="Error updating entity" description="Something went wrong while updating the entity." />,{
            unstyled: true,
          },
        );
        return;
      ...
      toast(<ToastPositive title={`${entityType} updated successfully!🎉`} />, {
        unstyled: true,
      });
    } catch (error) {
      toast(
  ...
  const companies = useMemo<ICompany[]>(() => {
    if (data && data.getEntities) {
      return data.getEntities
        .filter((entity) => entity?.__typename === LocalEntityType.COMPANY)
        .map((entity) => {
  ...
  const contacts = useMemo<IContact[]>(() => {
    if (data && data.getEntities) {
      return data.getEntities
        .filter((entity) => entity?.__typename === LocalEntityType.CONTACT)
        .map((entity) => {
